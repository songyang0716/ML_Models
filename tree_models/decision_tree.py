...
class DecisionTree:
    ...
    def predict(self, X):
        ...
        """
        return np.array([self._traverse(x, self.root, prob=False) for x in X])
    ...
    def _grow(self, X, Y):
        ...
        # continuous y, then return mean
        # categorical y, then return the prob for each class
    ...
    def _segment(self, X, Y, feat_idxs):
        ...
        for i in feat_idxs:
            ...
            if gains[np.argmax(gains)] >= best_gain:
                best_gain = gains[np.argmax(gains)]
                split_idx, split_thresh = i, thresholds[np.argmax(gains)]
...
def entropy(y):
    ...
    """
    ps = np.bincount(y) / len(y)
    return -1 * np.sum(np.array([p * np.log2(p) for p in ps if p != 0]))
